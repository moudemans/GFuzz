...
//        Vertex author = g.V().hasId(id).next();
        ...
        List<Edge> referees = author.getOutgoingEdges().stream().filter(edge -> edge.getLabel().equals("refers")).toList();
        ...
        List<Edge> referers = author.getIncomingEdges().stream().filter(edge -> edge.getLabel().equals("refers")).toList();
//        List<Edge> referers = g.V(author).inE("refers").limit(limit).toList();
        ...
        List<Edge> coauthors = author.getEdges().stream().filter(edge -> edge.getLabel().equals("collaborates")).toList();
