...
//                    Node hm_node = g.getNode((int) id);
//                    if(!hm_node.label.equals("homology_group")) {
//                        throw new RuntimeException();
//                    }
////                    test_if_correct_label(hm_node, HOMOLOGY_GROUP_LABEL, true);
//                    Iterable<Edge> relations = hm_node.getEdges();
//                    for (Edge rel : relations) {
...
//                    int genome_nr = Integer.parseInt(mrna_node.properties.get("genome"));
//
//                    temp_genes_per_genome.computeIfAbsent(genome_nr, i -> new ArrayList<>()).add(mrna_node);
//                }
//            }
...
//                test_if_correct_label(mrna_node, MRNA_LABEL, true);
    ...
    public static ArrayList<Node> findHmNodes(MyGraph g, int minimumMembers) throws RuntimeException {
        final ArrayList<Node> hmList = new ArrayList<>();
        ArrayList<Node> hmnodes = g.getNodes("homology_group");
        if (hmnodes == null) {
            throw new RuntimeException("");
        }
        Iterator<Node> hmNodes_it = hmnodes.iterator();
        while (hmNodes_it.hasNext()) {
            Node hmNode = hmNodes_it.next();
            int numMembers = Integer.parseInt(hmNode.getProperty("num_members"));
