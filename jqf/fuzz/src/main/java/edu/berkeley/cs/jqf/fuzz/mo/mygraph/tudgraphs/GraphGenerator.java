...
import static edu.berkeley.cs.jqf.fuzz.mo.mygraph.util.Util.computeTimePassed;
import static edu.berkeley.cs.jqf.fuzz.mo.mygraph.util.Util.dirExists;
...
//            MyGraph.writeGraphToFile(output_file_name, g);
    ...
    public static String generatePropertyValue(Property p) {
        ...
        if (p.type == Type.INT) {
            ...
            double random_val = r.nextDouble() - (Double.MIN_VALUE / 2);
            return random_val + "";
        } else if (p.type == Type.FLOAT) {
            float random_val = r.nextFloat() - (Float.MIN_VALUE / 2);
            return random_val + "";
