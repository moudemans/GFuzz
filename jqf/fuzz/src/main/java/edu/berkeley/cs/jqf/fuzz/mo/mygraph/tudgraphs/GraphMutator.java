...
import edu.berkeley.cs.jqf.fuzz.mo.mygraph.tudcomponents.*;
...
public class GraphMutator {
    ...
    private static String breakSchemaMutation(MyGraph g, Set<String> breaking_mutations) {
        ...
        List<GraphMutations.MutationMethod> active_breaking_mutations = breaking_mutations_methods.stream().filter(mutationMethod -> active_mutations.contains(mutationMethod)).collect(Collectors.toList());
    ...
    private static String breakCardinalityMutation(MyGraph g, Set<String> breaking_mutations) {
        ...
        ArrayList<Relationship> relationships_with_cardinality = new ArrayList<>(gs.getRelationships()
                .stream()
                .filter(relationship -> relationship.getCardinality() != Cardinality.MULTI).collect(Collectors.toList()));
        ...
        ArrayList<Relationship> filtered_relationships = new ArrayList<>(relationships_with_cardinality.stream().filter(relationship -> !cardinality_mutations_applied.contains(relToConstraintString(relationship))).collect(Collectors.toList()));
...
////        Property random_property = (random_index < node_labels.size()) ? node_labels.get(random_index) : edge_labels.get(random_index);
    ...
    private static Node getRandomNodeWithEdges(MyGraph g) {
        ...
        ArrayList<Node> filtered_nodes = new ArrayList<>(nodes.stream().filter(node -> !node.getEdges().isEmpty()).collect(Collectors.toList()));
    ...
    private static ArrayList<Node> getNodesWithProperties(MyGraph g) {
        ArrayList<Node> nodes = g.getNodes();
        ArrayList<Node> filtered_nodes = new ArrayList<>(nodes.stream().filter(node -> !node.properties.isEmpty()).collect(Collectors.toList()));
    ...
    private static ArrayList<Node> getNodesWithNonStringProperties(MyGraph g) {
        ...
        ArrayList<Node> nodes = g.getNodes();
        ArrayList<Node> filtered_nodes = new ArrayList<>(nodes.stream().filter(node -> non_string_properties_per_node.containsKey(node.label)).collect(Collectors.toList()));
    ...
    private static ArrayList<Edge> getEdgesWithProperties(MyGraph g) {
        ...
        ArrayList<Edge> filtered_edges = new ArrayList<>(edges.stream().filter(e -> !e.properties.isEmpty()).collect(Collectors.toList()));
    ...
    private static ArrayList<Edge> getEdgesWithNonStringProperties(MyGraph g) {
        ...
        ArrayList<Edge> edges = g.getEdges();
        ArrayList<Edge> filtered_edges = new ArrayList<>(edges.stream().filter(e -> non_string_properties_per_edge.containsKey(e.label)).collect(Collectors.toList()));
    ...
    private static void changePropertyType(MyGraph g) {
        ArrayList<Node> nodes = getNodesWithNonStringProperties(g);
        ArrayList<Edge> edges = getEdgesWithNonStringProperties(g);
        ...
        if (random_index < node_pool_size) {
            ...
            Set<String> non_string_property_labels = g.getSchema().getNodeProperties().get(n.label).stream().filter(property -> property != null && property.type != Type.STRING).map(property -> property.name).collect(Collectors.toSet());
            HashMap<String, String> non_string_properties = new HashMap<>();
            for (String p_label : n.properties.keySet()) {
//                if (non_string_property_labels.contains(p_label)) {
...
//                }
            ...
            Property old_prop = g.getSchema().getNodeProperties().get(n.label).stream().filter(property -> property != null && property.name.equals(prop_key)).findFirst().orElse(null);
...
//                }
            ...
            Set<String> non_string_property_labels = g.getSchema().getEdgeProperties().get(e.label).stream().filter(property -> property != null && property.type != Type.STRING).map(property -> property.name).collect(Collectors.toSet());
            HashMap<String, String> non_string_properties = new HashMap<>();
            ...
            Property old_prop = g.getSchema().getEdgeProperties().get(e.label).stream().filter(property -> property != null && property.name.equals(prop_key)).findFirst().orElse(null);
...
//                }
    ...
    private static void changePropertyValue(MyGraph g) {
        ...
        if (random_index < node_pool_size) {
            ...
            if (g.getSchema().getEdgeProperties() != null && g.getSchema().getEdgeProperties().get(e.label) != null) {
                p = g.getSchema().getEdgeProperties().get(e.label).stream().filter(property -> property != null && property.name.equals(prop_key)).findFirst().orElse(null);
            }
    ...
    private static GraphMutations.MutationMethod selectMutationMethod(Set<GraphMutations.MutationMethod> excludeMutations) {
        ...
        List<GraphMutations.MutationMethod> filtered = new ArrayList<>(mutations.stream().filter(mutationMethod -> !excludeMutations.contains(mutationMethod)).collect(Collectors.toList()));
